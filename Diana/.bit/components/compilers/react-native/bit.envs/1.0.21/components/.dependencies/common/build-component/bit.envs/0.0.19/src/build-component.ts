...
export function getBitAddCommand(files: Array<string>, compId: string): string {
	...
	bitAddCommand +=
		` --main ${files[0]} --id ${compId} --tests "${testsFiles}"`;
	return bitAddCommand;
}

export async function buildComponentInWorkspace(helper: Helper,
	opts?: BuildOptions): Promise<BuildResult> {
	const results: BuildResult = {directory: '', files: [], showComponent: {}};
	const component: GenericObject = (opts && opts.component) ||
		getDefaultComponent();
	...
	if (!opts || opts.disableBuildStep !== true) {
		output =
			helper.env.command.runCmd(getCommandString(compId, opts),
				results.directory);
	...
	results.showComponent =
		JSON.parse(helper.command.runCmd(`bit show ${compId} --json`,
			results.directory));
	return results;
}
...
export async function removeWorkspace(directory: string) {
	return new Promise((resolve, reject) => rimraf(directory, {},
